...
class CollegeInfoController extends Controller {
    ...
    async index() {
        ...
        //获取数据
        const result = await ctx.service.api.v1.collegeInfo.index(page, pageSize);
        //处理数据
    ...
    async show() {
        ...
        const {id} = ctx.params;
        //获取数据
        const result = await ctx.service.api.v1.collegeInfo.show(id);
        //处理数据
        ctx.status = result.status;//设置状态码
        ctx.set(result.header);//设置请求头
        ctx.body = result.data;//设置数据
...
module.exports = CollegeInfoController;
