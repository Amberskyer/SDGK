...
class schoolMajorAdmissionController extends Controller {
  ...
  async init() {
    ...
    async function initItem(offsetNum) {
      ...
      const SchoolMajorAdmissionResult = await ctx.cSchoolMajorAdmissionModel.SchoolMajorAdmissionTable.findAll({
        where: {
          status: -1,
          year: {
            $in: [ 2017, 2018 ],
      ...
      for (let i = 0; i < SchoolMajorAdmissionResult.length; i++) {
        ...
        if (SchoolMajorAdmissionInfo) {
          ...
          const result = await ctx.sdgkModel.SchoolMajorAdmission.findAll({
            where: {
        ...
        if (numCount === 2) {
          ...
          await ctx.cSchoolMajorAdmissionModel.SchoolMajorAdmissionTable.update({
            status: 404,
          }, {
            where: {
              id: {
  ...
  async initSchoolMajorAdmissionTable() {
      ...
      for (let j = 0; j < this.provinceList.length; j++) {
        ...
        for (let k = 0; k < this.subjectTypeList.length; k++) {
          ...
          for (let l = 0; l < this.yearList.length; l++) {
            ...
            // for (let m = 0; m < this.batchList.length; m++) {
            //   const batchItem = this.batchList[m];
            //
            //
            // }
    ...
    await ctx.cSchoolMajorAdmissionModel.SchoolMajorAdmissionTable.bulkCreate(schoolAdmissionJson);
    console.log('结束');
